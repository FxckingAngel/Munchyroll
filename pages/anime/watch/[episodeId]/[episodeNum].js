...
function StreamingPage({ episode, anime, episodeNumber, dub }) {
  ...
  const { data: streamLink, isLoading, isError, error } = useQuery(["details", episodeName], () =>
    getAnimeEpisodeLinks(episodeName)
  );

  const handleVLCPlayer = () => {
    window.open(
      `intent:${streamLink?.sources[0].file}#Intent;scheme=vlc;package=org.videolan.vlc;end`,
      "_blank",
    );
  };
  ...
  const handleMxPlayer = () => {
    window.open(
      `intent:${streamLink?.sources[0].file}#Intent;scheme=mxplayer;package=com.mxtech.videoplayer.ad;end`,
      "_blank",
    );
  };
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:title" content={"Watch " + (anime?.title.english || anime?.title.romaji) + " Episode " + episodeNumber + " - Munchyroll "} />
          ...
          <div className="mt-3 lg:flex lg:space-x-4 rounded-xl">
            <div className="alignfull w-full overflow-hidden max-w-screen-xl rounded-xl">
              {isDubbed ? (
                ...
                <VideoPlayer videoSource={streamLink?.sources[0].file} key={streamLink?.sources[0].file} className="rounded-xl " />
              )}
              ...
              <div className="flex justify-between pt-5">
                <Link className={`justify-start ${(episodeNumber > 1) ? "" : "invisible"}`} href={`/anime/watch/${anime.id}/${episodeNumber - 1}`}>
                  <button title="Go to the previous episode" className="bg-[#2f6b91] hover:bg-[#214861] transition-all text-white font-bold m-4 py-2 px-4 rounded">&#x2190; Episode {episodeNumber - 1}</button>
                </Link>
                <Link className={`justify-end ${(episodeNumber < episode.length) ? "" : "invisible"}`} href={`/anime/watch/${anime.id}/${episodeNumber + 1}`}>
                  <button title="Go to the next episode" className="bg-[#2f6b91] hover:bg-[#214861] transition-all text-white font-bold m-4 py-2 px-4 rounded">Episode {episodeNumber + 1} &#x2192;</button>
                </Link>
              </div>
